...
public class MailGenerator {
    ...
    public MailGenerator(int mailToCreate, int mailMaxWeight,
                         MailPool mailPool, HashMap<Boolean,Integer> seed) {
    ...
    private int generateDestinationFloor(){
        return Building.LOWEST_FLOOR +
                random.nextInt(Building.FLOORS);
    }
    ...
    public void generateAllMail(){
        while(!complete){
            ...
            if(allMail.containsKey(timeToDeliver)){
                ...
                /** If the key doesn't exist then set a new key along
                 *  with the array of MailItems to add during
                 * that time step.
                 */
